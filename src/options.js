...
function updateTonePreview() {
  ...
  tonePromptPreview.textContent = "";
  const strong = document.createElement("strong");
  strong.textContent = "Prompt sent to AI: ";
  tonePromptPreview.appendChild(strong);

  if (promptText) {
    const code = document.createElement("code");
    code.textContent = promptText;
    tonePromptPreview.appendChild(code);
  } else {
    const em = document.createElement("em");
    em.className = "muted";
    em.textContent = "No tone adjustment — just fix grammar & vocabulary";
    tonePromptPreview.appendChild(em);
  }
}
...
async function loadDisabledList() {
  ...
  if (disabledUntil > now) {
    ...
    label.textContent = "⏱️ Everywhere (timer)";
    timerItem.appendChild(label);

    const btn = document.createElement("button");
    btn.type = "button";
    btn.dataset.clear = "timer";
    btn.className = "remove-btn";
    btn.textContent = "×";
    timerItem.appendChild(btn);
  ...
  disabledSites.forEach(site => {
    ...
    const btn = document.createElement("button");
    btn.type = "button";
    btn.dataset.clear = "site";
    btn.dataset.value = site;
    btn.className = "remove-btn";
    btn.textContent = "×";
  ...
  disabledPages.forEach(page => {
    ...
    const btn = document.createElement("button");
    btn.type = "button";
    btn.dataset.clear = "page";
    btn.dataset.value = page;
    btn.className = "remove-btn";
    btn.textContent = "×";
    item.appendChild(btn);
